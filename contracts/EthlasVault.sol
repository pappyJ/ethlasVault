...
contract ETHLASVAULT is Ownable {
  ...
  event TokenAllowed(address indexed tokenAddress, bool allowed, uint8 decimals, string symbol);
   ...
   */
  function allowToken(address _tokenAddress, bool _allowed, uint8 _decimals, string memory _symbol) public onlyOwner {
    tokenInfo[_tokenAddress].allowed = _allowed;
    tokenInfo[_tokenAddress].decimals = _decimals;
    tokenInfo[_tokenAddress].symbol = _symbol;
    emit TokenAllowed(_tokenAddress, _allowed, _decimals, _symbol);
  }
  ...
  function getDepositInfo(address _account, address _tokenAddress) public view returns (uint256, uint256) {
    (uint256 amount, uint256 depositTime) = (_deposits[_tokenAddress][_account].amount, _deposits[_tokenAddress][_account].depositTime);
    return (amount, depositTime);
  }
   ...
   */
  function getTokenInfo(address _tokenAddress) public view returns (bool, uint8, string memory) {
    return (tokenInfo[_tokenAddress].allowed, tokenInfo[_tokenAddress].decimals, tokenInfo[_tokenAddress].symbol);
  }
}
